...
select docid,  id,0.8 as C1, fundingclass1 from (
...
select docid, id,sqroot(min(1.49,confidence)/1.5) as c1, fundingclass1 from ( select docid,id,max(confidence) as confidence, small_string, string, fundingclass1 from ( select docid, id,
    (fullprojectmatch*10
    +coreprojectmatch*10
...
select docid, id,sqroot(min(1.49,confidence)/1.5) as conf, fundingclass1 from ( select docid,id,max(confidence) as confidence, fundingClass1 from ( select docid, id, fundingClass1,
...
jdict("@graph", jgroup(
  	           jdict
    			("@id","funding1Uri",
     			"@type","http://openminted.eu/ns/fi#Funding", 
      			"http://openminted.eu/ns/fi#confidence", c1, 
      			"fund",fundingclass1,
      			"grantId",id)),
      "@context",jdict("funding-info",jdict("@id","http://openminted.eu/ns/fi#funding-info",
                                            "@type","oa:Annotation"),
                       "confidence",jdict("@id","http://openminted.eu/ns/fi#confidence",
                                          "@type","http://www.w3.org/2001/XMLSchema#double"),
                       "grantid",jdict("@id","http://openminted.eu/ns/fi#grantId"),
                       "fund",jdict("@id","http://openminted.eu/ns/fi#fund"),
